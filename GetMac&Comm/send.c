...
int main()
{
	...
	//发送序列的长度  
	size = sizeof(struct EthernetHeader) + sizeof(struct IpHeader) + sizeof(struct TcpHeader) + strlen(TcpData);  
	result = pcap_sendpacket(adhandle, SendBuffer,size);  
	if (result != 0)  
	{  
		printf("Send Error!\n");  
	}   
...
{  
    ...
    int i = -1;  
    int res;  
    struct EthernetHeader eh; //以太网帧头  
    struct Arpheader ah;  //ARP帧头  
...
{  
    ...
    struct pcap_t *adhandle = spara->adhandle;  
    char *ip = spara->ip;  
...
{  
    struct gparam *gpara = (struct gparam *) lpParameter;  
    struct pcap_t *adhandle = gpara->adhandle;  
    int res;  
    unsigned char Mac[6];  
    struct pcap_pkthdr * pkt_header;  
    const u_char * pkt_data;  
	...
	{  
		...
		{  
            if (*(unsigned short *) (pkt_data + 12) == htons(ETH_ARP))
			{  
                struct ArpPacket *recv = (struct ArpPacket *) pkt_data;  
